...
- `netlify/functions/evaluate.js`: función serverless que llama a Anthropic y guarda resultados.
...
| Nombre | Valor | Scope |
|---|---|---|
| `ANTHROPIC_API_KEY` | Tu key de Anthropic, inicia con `sk-ant-...` | Functions |
| `ANTHROPIC_MODEL` | `claude-sonnet-4-6` | Functions |
...
Si `hasAnthropicKey` sale `false`, la API key no quedó bien configurada o falta redeploy.
...
Cada evaluación llama a la API de Anthropic y consume créditos. Para 30 personas, configura límites de gasto en la consola de Anthropic antes del taller.
